------------
-- RUSSIAN --
------------
--Last update: 10.06.2010 - 09:35

if (GetLocale() == "ruRU") then
    -------------------
    -- Compatibility --
    -------------------

    HEALBOT_DRUID                          = "Друид";
    HEALBOT_HUNTER                         = "Охотник";
    HEALBOT_MAGE                           = "Маг";
    HEALBOT_PALADIN                        = "Паладин";
    HEALBOT_PRIEST                         = "Жрец";
    HEALBOT_ROGUE                          = "Разбойник";
    HEALBOT_SHAMAN                         = "Шаман";
    HEALBOT_WARLOCK                        = "Чернокнижник";
    HEALBOT_WARRIOR                        = "Воин";
    HEALBOT_DEATHKNIGHT                    = "Рыцарь cмерти";
    HEALBOT_NEW_ITEMS                      = "Предметы";

    --ITEMS--

    HEALBOT_HAND_OF_THE_GUARDIANS          = GetSpellInfo(313104);  --Длань Хранителя
    HEALBOT_TOUCH_OF_THE_TITANS            = GetSpellInfo(313106);  --Касание Титанов
    HEALBOT_ACCESSORY_HEALING              = GetSpellInfo(311060);  --Целительное зелье

    --NEW_SPELLS--
    
    HEALBOT_COVER_OF_LIGHT                 = GetSpellInfo(319738);  --Покров света
    HEALBOT_RESIDUAL_SHINE                 = GetSpellInfo(319540);  --Остаточное сияние (T6)
    HEALBOT_THE_GIFT_OF_ISERA              = GetSpellInfo(308090);  --Дар Изеры
    HEALBOT_FLOWERING_PERIOD               = GetSpellInfo(308093);  --Период цветения
    HEALBOT_DIVINE_HAND                    = GetSpellInfo(319166);  --Божья длань
    HEALBOT_BLOSSOMING                     = GetSpellInfo(308094);  --Расцвет
    HEALBOT_RENEWAL                        = GetSpellInfo(305081);  --Обновление (Т4)
    HEALBOT_AURA_MASTER                    = GetSpellInfo(31821);   --Мастер аур
    HEALBOT_DIVINE_GUARDIAN                = GetSpellInfo(70940);   --Божественный страж
    HEALBOT_SACRED_SACRIFICE               = GetSpellInfo(64205);   --Священная жертва
    HEALBOT_HEALING_RAIN                   = GetSpellInfo(308017);  --Целительный Ливень
    HEALBOT_PRIMEVAL_MOISTURE              = GetSpellInfo(308015);  --Первозданная влага
    HEALBOT_ICE_BLOCK                      = GetSpellInfo(45438);   --Ледяная глыба
    

    --STANDART_SPELLS--

    HEALBOT_SILK_BANDAGE                   = GetItemInfo(6450);     --Шелковые бинты
    HEALBOT_HEAVY_SILK_BANDAGE             = GetItemInfo(6451);     --Плотные шелковые бинты
    HEALBOT_MAGEWEAVE_BANDAGE              = GetItemInfo(8544);     --Бинты из магической ткани
    HEALBOT_HEAVY_MAGEWEAVE_BANDAGE        = GetItemInfo(8545);     --Плотные бинты из магической ткани
    HEALBOT_RUNECLOTH_BANDAGE              = GetItemInfo(14529);    --Бинты из рунической ткани
    HEALBOT_HEAVY_RUNECLOTH_BANDAGE        = GetItemInfo(14530);    --Плотные бинты из рунной ткани
    HEALBOT_NETHERWEAVE_BANDAGE            = GetItemInfo(21990);    --Бинты из ткани Пустоты
    HEALBOT_HEAVY_NETHERWEAVE_BANDAGE      = GetItemInfo(21991);    --Плотные бинты из ткани Пустоты
    HEALBOT_FROSTWEAVE_BANDAGE             = GetItemInfo(34721);    --Бинты из ледяной ткани
    HEALBOT_HEAVY_FROSTWEAVE_BANDAGE       = GetItemInfo(34722);    --Плотные бинты из ледяной ткани
    HEALBOT_MAJOR_HEALING_POTION           = GetItemInfo(13446);    --Хорошее лечебное зелье
    HEALBOT_SUPER_HEALING_POTION           = GetItemInfo(22829);    --Гигантский флакон с лечебным зельем
    HEALBOT_MAJOR_COMBAT_HEALING_POTION    = GetItemInfo(31838);    --Большой флакон с боевым лечебным зельем
    HEALBOT_RUNIC_HEALING_POTION           = GetItemInfo(33447);    --Рунический флакон с лечебным зельем
    HEALBOT_ENDLESS_HEALING_POTION         = GetItemInfo(43569);    --Бездонный флакон с лечебным зельем
    HEALBOT_MAJOR_MANA_POTION              = GetItemInfo(13444);    --Огромный флакон с зельем маны
    HEALBOT_SUPER_MANA_POTION              = GetItemInfo(22832);    --Гигантский флакон с зельем маны
    HEALBOT_MAJOR_COMBAT_MANA_POTION       = GetItemInfo(31840);    --Большой флакон с боевым зельем маны
    HEALBOT_RUNIC_MANA_POTION              = GetItemInfo(33448);    --Рунический флакон с зельем маны
    HEALBOT_ENDLESS_MANA_POTION            = GetItemInfo(43570);    --Бездонный флакон с зельем маны
    HEALBOT_PURIFICATION_POTION            = GetItemInfo(13462);    --Зелье очищения
    HEALBOT_ANTI_VENOM                     = GetItemInfo(6452);     --Противоядие
    HEALBOT_POWERFUL_ANTI_VENOM            = GetItemInfo(19440);    --Мощное противоядие
    HEALBOT_ELIXIR_OF_POISON_RES           = GetItemInfo(3386);     --Эликсир излечения
    HEALBOT_STONEFORM                      = GetSpellInfo(20594);
    HEALBOT_ANCESTRAL_FORTITUDE            = GetSpellInfo(16237);
    HEALBOT_INSPIRATION                    = GetSpellInfo(14892);
    HEALBOT_WATER_BREATHING                = GetSpellInfo(131);
    HEALBOT_WATER_WALKING                  = GetSpellInfo(546);
    HEALBOT_EARTHLIVING                    = GetSpellInfo(51945);

    HEALBOT_GIFT_OF_THE_NAARU              = GetSpellInfo(59547);

    HEALBOT_FLASH_HEAL                     = GetSpellInfo(2061);    --Быстрое исцеление
    HEALBOT_FLASH_OF_LIGHT                 = GetSpellInfo(19750);   --Вспышка Света
    HEALBOT_GREATER_HEAL                   = GetSpellInfo(2060);    --Великое исцеление
    HEALBOT_BINDING_HEAL                   = GetSpellInfo(32546);   --Связующее исцеление
    HEALBOT_PENANCE                        = GetSpellInfo(47540);   --Искупление
    HEALBOT_PRAYER_OF_MENDING              = GetSpellInfo(33076);   --Молитва восстановления
    HEALBOT_HEALING_TOUCH                  = GetSpellInfo(5185);    --Целительное прикосновение
    HEALBOT_HEAL                           = GetSpellInfo(2054);    --Исцеление
    HEALBOT_HEALING_WAVE                   = GetSpellInfo(331);     --Волна исцеления
    HEALBOT_RIPTIDE                        = GetSpellInfo(61295);   --Быстрина
    HEALBOT_HEALING_WAY                    = GetSpellInfo(29206);   --Путь исцеления
    HEALBOT_HOLY_LIGHT                     = GetSpellInfo(635);     --Свет небес
    HEALBOT_LESSER_HEAL                    = GetSpellInfo(2050);    --Малое исцеление
    HEALBOT_LESSER_HEALING_WAVE            = GetSpellInfo(8004);    --Малая волна исцеления
    HEALBOT_POWER_WORD_SHIELD              = GetSpellInfo(17);
    HEALBOT_REGROWTH                       = GetSpellInfo(8936);    --Восстановление
    HEALBOT_RENEW                          = GetSpellInfo(139);     --Обновление
    HEALBOT_REJUVENATION                   = GetSpellInfo(774);     --Омоложение
    HEALBOT_LIFEBLOOM                      = GetSpellInfo(33763);   --Жизнецвет
    HEALBOT_WILD_GROWTH                    = GetSpellInfo(48438);   --Буйный рост
    HEALBOT_REVIVE                         = GetSpellInfo(50769);   --Оживление
    HEALBOT_TRANQUILITY                    = GetSpellInfo(740);     --Спокойствие
    HEALBOT_SWIFTMEND                      = GetSpellInfo(18562);   --Быстрое восстановление
    HEALBOT_LIVING_SEED                    = GetSpellInfo(48496);
    HEALBOT_PRAYER_OF_HEALING              = GetSpellInfo(596);     --Молитва исцеления
    HEALBOT_CHAIN_HEAL                     = GetSpellInfo(1064);    --Цепное исцеление
    HEALBOT_NOURISH                        = GetSpellInfo(50464);   --Покровительство Природы
    HEALBOT_PROTANCIENTKINGS               = GetSpellInfo(64413);
    HEALBOT_FOUNTAIN_OF_LIGHT              = GetSpellInfo(71864);
    HEALBOT_HANDOFPROTECTION               = GetSpellInfo(1022);
    HEALBOT_ENERGIZED                      = GetSpellInfo(71220);   --Заряженный
    HEALBOT_CHAINHEALHOT                   = GetSpellInfo(70809);   --Дополнительное цепное исцеление

    HEALBOT_GRACE                          = GetSpellInfo(47516);
    HEALBOT_GUARDIAN_SPIRIT                = GetSpellInfo(47788);
    HEALBOT_LEVITATE                       = GetSpellInfo(1706);
    HEALBOT_DIVINE_AEGIS                   = GetSpellInfo(47509);
    HEALBOT_BODY_AND_SOUL                  = GetSpellInfo(64127);
    HEALBOT_SURGE_OF_LIGHT                 = GetSpellInfo(33154);
    HEALBOT_CIRCLE_OF_HEALING              = GetSpellInfo(34863);
    HEALBOT_BLESSED_HEALING                = GetSpellInfo(70772);
    HEALBOT_BLESSED_RESILIENCE             = GetSpellInfo(33142);   --Благословенная устойчивость
    HEALBOT_PAIN_SUPPRESSION               = GetSpellInfo(33206);   --Подавление боли
    HEALBOT_POWER_INFUSION                 = GetSpellInfo(10060);   --Придание сил
    HEALBOT_POWER_WORD_FORTITUDE           = GetSpellInfo(1243);    --Слово силы: Стойкость
    HEALBOT_PRAYER_OF_FORTITUDE            = GetSpellInfo(21562);   --Молитва стойкости
    HEALBOT_DIVINE_SPIRIT                  = GetSpellInfo(14752);   --Божественный дух
    HEALBOT_PRAYER_OF_SPIRIT               = GetSpellInfo(27681);   --Молитва духа
    HEALBOT_SHADOW_PROTECTION              = GetSpellInfo(976);     --Защита от темной магии
    HEALBOT_PRAYER_OF_SHADOW_PROTECTION    = GetSpellInfo(27683);   --Молитва защиты от темной магии
    HEALBOT_INNER_FIRE                     = GetSpellInfo(588);     --Внутренний огонь
    HEALBOT_SHADOWFORM                     = GetSpellInfo(15473);   --Облик Тьмы
    HEALBOT_INNER_FOCUS                    = GetSpellInfo(14751);   --Внутреннее сосредоточение
    HEALBOT_TWIN_DISCIPLINES               = GetSpellInfo(47586);   --Двоевластие
    HEALBOT_SPIRITAL_HEALING               = GetSpellInfo(14898);   --Духовное исцеление
    HEALBOT_SPIRITAL_GUIDANCE              = GetSpellInfo(14901);   --Духовное направление
    HEALBOT_EMPOWERED_HEALING              = GetSpellInfo(33158);   --Усиленное исцеление
    HEALBOT_DIVINE_PROVIDENCE              = GetSpellInfo(47562);   --Божественное провидение
    HEALBOT_IMPROVED_RENEW                 = GetSpellInfo(14908);   --Улучшенное обновление
    HEALBOT_EMPOWERED_RENEW                = GetSpellInfo(63534);   --Усиленное обновление
    HEALBOT_FOCUSED_POWER                  = GetSpellInfo(33186);   --Средоточие Силы
    HEALBOT_GENESIS                        = GetSpellInfo(57810);   --Сотворение
    HEALBOT_NURTURING_INSTINCT             = GetSpellInfo(33872);   --Материнский инстинкт
    HEALBOT_IMPROVED_REJUVENATION          = GetSpellInfo(17111);   --Улучшенное омоложение
    HEALBOT_GIFT_OF_NATURE                 = GetSpellInfo(17104);   --Дар природы
    HEALBOT_EMPOWERED_TOUCH                = GetSpellInfo(33879);   --Усиленное прикосновение
    HEALBOT_EMPOWERED_REJUVENATION         = GetSpellInfo(33886);   --Усиленное омоложение
    HEALBOT_MASTER_SHAPESHIFTER            = GetSpellInfo(48412);   --Искусный оборотень
    HEALBOT_HEALING_LIGHT                  = GetSpellInfo(20237);   --Исцеляющий Свет
    HEALBOT_DIVINITY                       = GetSpellInfo(63646);   --Божественность
    HEALBOT_TOUCHED_BY_THE_LIGHT           = GetSpellInfo(53590);   --Касание Света
    HEALBOT_HOLY_GUIDANCE                  = GetSpellInfo(31841);
    HEALBOT_PURIFICATION                   = GetSpellInfo(16178);   --Чистота
    HEALBOT_IMPROVED_CHAIN_HEAL            = GetSpellInfo(30872);   --Улучшенное цепное исцеление
    HEALBOT_TIDAL_WAVES                    = GetSpellInfo(51562);   --Приливные волны
    HEALBOT_TIDAL_FORCE                    = GetSpellInfo(55198);   --Сила прилива
    HEALBOT_NATURES_BLESSING               = GetSpellInfo(30867);   --Благословение природы
    HEALBOT_HEALTH_FUNNEL                  = GetSpellInfo(755);
    HEALBOT_LIFE_TAP                       = GetSpellInfo(1454);
    HEALBOT_FEAR_WARD                      = GetSpellInfo(6346);    --Защита от страха
    HEALBOT_ARCANE_INTELLECT               = GetSpellInfo(1459);    --Чародейский интеллект
    HEALBOT_ARCANE_BRILLIANCE              = GetSpellInfo(23028);   --Чародейская гениальность
    HEALBOT_DALARAN_INTELLECT              = GetSpellInfo(61024);   --Даларанский интеллект
    HEALBOT_DALARAN_BRILLIANCE             = GetSpellInfo(61316);   --Чародейская гениальность Даларана
    HEALBOT_FROST_ARMOR                    = GetSpellInfo(168);     --Морозный доспех
    HEALBOT_ICE_ARMOR                      = GetSpellInfo(7302);    --Ледяной доспех
    HEALBOT_MAGE_ARMOR                     = GetSpellInfo(6117);    --Магический доспех
    HEALBOT_MOLTEN_ARMOR                   = GetSpellInfo(30482);   --Раскаленный доспех
    HEALBOT_DEMON_ARMOR                    = GetSpellInfo(706);     --Демонический доспех
    HEALBOT_DEMON_SKIN                     = GetSpellInfo(687);     --Шкура демона
    HEALBOT_FEL_ARMOR                      = GetSpellInfo(28176);   --Доспех Скверны
    HEALBOT_SOUL_LINK                      = GetSpellInfo(19028);   --Связка души
    HEALBOT_DAMPEN_MAGIC                   = GetSpellInfo(604);     --Ослабление магии
    HEALBOT_AMPLIFY_MAGIC                  = GetSpellInfo(1008);    --Усиление магии
    HEALBOT_DETECT_INV                     = GetSpellInfo(132);     --Обнаружение невидимости
    HEALBOT_FOCUS_MAGIC                    = GetSpellInfo(54646);   --Магическая концентрация
    HEALBOT_NATURE_SWIFTNESS               = GetSpellInfo(17116);   --Природная стремительность

    HEALBOT_LIGHTNING_SHIELD               = GetSpellInfo(324);     --Щит молний
    HEALBOT_ROCKBITER_WEAPON               = GetSpellInfo(8017);    --Оружие Камнедробителя
    HEALBOT_FLAMETONGUE_WEAPON             = GetSpellInfo(8024);    --Оружие языка пламени
    HEALBOT_EARTHLIVING_WEAPON             = GetSpellInfo(51730);   --Оружие жизни Земли
    HEALBOT_WINDFURY_WEAPON                = GetSpellInfo(8232);    --Оружие неистовства ветра
    HEALBOT_FROSTBRAND_WEAPON              = GetSpellInfo(8033);    --Оружие ледяного клейма
    HEALBOT_EARTH_SHIELD                   = GetSpellInfo(974);     --Щит земли
    HEALBOT_WATER_SHIELD                   = GetSpellInfo(52127);   --Водный щит

    HEALBOT_MARK_OF_THE_WILD               = GetSpellInfo(1126);    --Знак дикой природы
    HEALBOT_GIFT_OF_THE_WILD               = GetSpellInfo(21849);   --Дар дикой природы
    HEALBOT_THORNS                         = GetSpellInfo(467);     --Шипы
    HEALBOT_OMEN_OF_CLARITY                = GetSpellInfo(16864);   --Знамение ясности

    HEALBOT_BEACON_OF_LIGHT                = GetSpellInfo(53563);   --Частица света
    HEALBOT_LIGHT_BEACON                   = GetSpellInfo(53651);
    HEALBOT_SACRED_SHIELD                  = GetSpellInfo(53601);   --Священный щит
    HEALBOT_SACRED_CLEANSING               = GetSpellInfo(53551);   --Священное очищение
    HEALBOT_LAY_ON_HANDS                   = GetSpellInfo(27154);   --Возложение рук
    HEALBOT_IMPROVED_LAY_ON_HANDS          = GetSpellInfo(27154);   -- Improved didnt show icon - Aura has 'Lay On Hands' and id 27154 works.  Improved is GetSpellInfo(20234)
    HEALBOT_INFUSION_OF_LIGHT              = GetSpellInfo(53569);
    HEALBOT_HOLY_SHOCK                     = GetSpellInfo(20473);
    HEALBOT_SERENDIPITY                    = GetSpellInfo(63730);
    HEALBOT_DIVINE_FAVOR                   = GetSpellInfo(20216);
    HEALBOT_DIVINE_ILLUMINATION            = GetSpellInfo(31842);
    HEALBOT_DIVINE_SHIELD                  = GetSpellInfo(642);
    HEALBOT_RIGHTEOUS_DEFENSE              = GetSpellInfo(31789);
    HEALBOT_SHEATH_OF_LIGHT                = GetSpellInfo(53501);   --Покров Света
    HEALBOT_BLESSED                        = GetSpellInfo(71192);   --Благословенность
    HEALBOT_BLESSING_OF_MIGHT              = GetSpellInfo(19740);   --Благословение могущества
    HEALBOT_BLESSING_OF_WISDOM             = GetSpellInfo(19742);   --Благословение мудрости
    HEALBOT_BLESSING_OF_SANCTUARY          = GetSpellInfo(20911);   --Благословение неприкосновенности
    HEALBOT_BLESSING_OF_PROTECTION         = GetSpellInfo(41450);   --Благословение защиты
    HEALBOT_BLESSING_OF_KINGS              = GetSpellInfo(20217);   --Благословение королей
    HEALBOT_GREATER_BLESSING_OF_MIGHT      = GetSpellInfo(25782);   --Великое благословение могущества
    HEALBOT_GREATER_BLESSING_OF_WISDOM     = GetSpellInfo(25894);   --Великое благословение мудрости
    HEALBOT_GREATER_BLESSING_OF_KINGS      = GetSpellInfo(25898);   --Великое благословение королей
    HEALBOT_GREATER_BLESSING_OF_SANCTUARY  = GetSpellInfo(25899);   --Великое благословение неприкосновенности
    HEALBOT_SEAL_OF_RIGHTEOUSNESS          = GetSpellInfo(21084);   --Печать праведности
    HEALBOT_SEAL_OF_BLOOD                  = GetSpellInfo(31892);   --Печать крови
    HEALBOT_SEAL_OF_CORRUPTION             = GetSpellInfo(53736);   --Печать порчи
    HEALBOT_SEAL_OF_JUSTICE                = GetSpellInfo(20164);   --Печать справедливости
    HEALBOT_SEAL_OF_LIGHT                  = GetSpellInfo(20165);   --Печать Cвета
    HEALBOT_SEAL_OF_VENGEANCE              = GetSpellInfo(31801);   --Печать мщения
    HEALBOT_SEAL_OF_WISDOM                 = GetSpellInfo(20166);   --Печать мудрости
    HEALBOT_SEAL_OF_COMMAND                = GetSpellInfo(20375);   --Печать повиновения
    HEALBOT_SEAL_OF_THE_MARTYR             = GetSpellInfo(53720);   --Печать мученика
    HEALBOT_HAND_OF_FREEDOM                = GetSpellInfo(1044);    --Длань свободы
    HEALBOT_HAND_OF_PROTECTION             = GetSpellInfo(1022);    --Длань защиты
    HEALBOT_HAND_OF_SACRIFICE              = GetSpellInfo(6940);    --Длань жертвенности
    HEALBOT_HAND_OF_SALVATION              = GetSpellInfo(1038);    --Длань спасения
    HEALBOT_RIGHTEOUS_FURY                 = GetSpellInfo(25780);   --Праведное неистовство
    HEALBOT_DEVOTION_AURA                  = GetSpellInfo(465);     --Аура благочестия
    HEALBOT_RETRIBUTION_AURA               = GetSpellInfo(7294);    --Аура возмездия
    HEALBOT_CONCENTRATION_AURA             = GetSpellInfo(19746);   --Аура сосредоточенности
    HEALBOT_SHR_AURA                       = GetSpellInfo(19876);   --Аура защиты от темной магии
    HEALBOT_FRR_AURA                       = GetSpellInfo(19888);   --Аура защиты от магии льда
    HEALBOT_FIR_AURA                       = GetSpellInfo(19891);   --Аура защиты от огня
    HEALBOT_CRUSADER_AURA                  = GetSpellInfo(32223);   --Аура воина Света

    HEALBOT_BATTLE_SHOUT                   = GetSpellInfo(6673);
    HEALBOT_COMMANDING_SHOUT               = GetSpellInfo(469);
    HEALBOT_INTERVENE                      = GetSpellInfo(3411);
    HEALBOT_VAMPIRIC_EMBRACE               = GetSpellInfo(15286);
    HEALBOT_VIGILANCE                      = GetSpellInfo(50720);   --Бдительность
    HEALBOT_LAST_STAND                     = GetSpellInfo(12975);
    HEALBOT_SHIELD_WALL                    = GetSpellInfo(871);
    HEALBOT_SHIELD_BLOCK                   = GetSpellInfo(2565);
    HEALBOT_ENRAGED_REGEN                  = GetSpellInfo(55694);
    HEALBOT_DIVINE_PROTECTION              = GetSpellInfo(498);
    HEALBOT_BARKSKIN                       = GetSpellInfo(22812);
    HEALBOT_SURVIVAL_INSTINCTS             = GetSpellInfo(61336);
    HEALBOT_FRENZIED_REGEN                 = GetSpellInfo(22842);
    HEALBOT_ICEBOUND_FORTITUDE             = GetSpellInfo(48792);
    HEALBOT_ANTIMAGIC_SHELL                = GetSpellInfo(48707);
    HEALBOT_ARMY_OF_THE_DEAD               = GetSpellInfo(42650);
    HEALBOT_LICHBORNE                      = GetSpellInfo(49039);
    HEALBOT_ANTIMAGIC_ZONE                 = GetSpellInfo(51052);
    HEALBOT_VAMPIRIC_BLOOD                 = GetSpellInfo(55233);
    HEALBOT_UNBREAKABLE_ARMOR              = GetSpellInfo(51271);
    HEALBOT_BONE_SHIELD                    = GetSpellInfo(49222);
    HEALBOT_HORN_OF_WINTER                 = GetSpellInfo(57330);
    HEALBOT_HYSTERIA                       = GetSpellInfo(49016);
    HEALBOT_INNERVATE                      = GetSpellInfo(29166);

    HEALBOT_A_MONKEY                       = GetSpellInfo(13163);   --Дух обезьяны
    HEALBOT_A_HAWK                         = GetSpellInfo(13165);   --Дух ястреба
    HEALBOT_A_CHEETAH                      = GetSpellInfo(5118);    --Дух гепарда
    HEALBOT_A_BEAST                        = GetSpellInfo(13161);   --Дух зверя
    HEALBOT_A_PACK                         = GetSpellInfo(13159);   --Дух стаи
    HEALBOT_A_WILD                         = GetSpellInfo(20043);   --Дух дикой природы
    HEALBOT_A_VIPER                        = GetSpellInfo(34074);   --Дух гадюки
    HEALBOT_A_DRAGONHAWK                   = GetSpellInfo(61846);   --Дух дракондора
    HEALBOT_MENDPET                        = GetSpellInfo(136);     --Лечение питомца

    HEALBOT_UNENDING_BREATH                = GetSpellInfo(5697);    --Нескончаемое дыхание

    HEALBOT_RESURRECTION                   = GetSpellInfo(2006);    --Воскрешение
    HEALBOT_REDEMPTION                     = GetSpellInfo(7328);    --Искупление
    HEALBOT_REBIRTH                        = GetSpellInfo(20484);   --Возрождение
    HEALBOT_ANCESTRALSPIRIT                = GetSpellInfo(2008);    --Дух предков

    HEALBOT_PURIFY                         = GetSpellInfo(1152);    --Омовение
    HEALBOT_CLEANSE                        = GetSpellInfo(4987);    --Очищение
    HEALBOT_CURE_POISON                    = GetSpellInfo(8946);    --Выведение яда
    HEALBOT_REMOVE_CURSE                   = GetSpellInfo(2782);    --Снятие проклятия
    HEALBOT_ABOLISH_POISON                 = GetSpellInfo(2893);    --Устранение яда
    HEALBOT_CURE_DISEASE                   = GetSpellInfo(528);     --Излечение болезни
    HEALBOT_ABOLISH_DISEASE                = GetSpellInfo(552);     --Устранение болезни
    HEALBOT_DISPEL_MAGIC                   = GetSpellInfo(527);     --Рассеивание заклинаний
    HEALBOT_CLEANSE_SPIRIT                 = GetSpellInfo(51886);   --Очищение духа
    HEALBOT_CURE_TOXINS                    = GetSpellInfo(526);     --Оздоровление
    HEALBOT_DISEASE                        = "Болезнь";
    HEALBOT_MAGIC                          = "Магия";
    HEALBOT_CURSE                          = "Проклятие";
    HEALBOT_POISON                         = "Яд";
    -- HEALBOT_DISEASE_en         = "Disease";  -- Do NOT localize this value.
    -- HEALBOT_MAGIC_en           = "Magic";  -- Do NOT localize this value.
    -- HEALBOT_CURSE_en           = "Curse";  -- Do NOT localize this value.
    -- HEALBOT_POISON_en          = "Poison";  -- Do NOT localize this value.
    -- HEALBOT_CUSTOM_en          = "Custom";  -- Do NOT localize this value.

    HEALBOT_DEBUFF_ANCIENT_HYSTERIA        = "Древняя истерия";
    HEALBOT_DEBUFF_IGNITE_MANA             = "Воспламенение маны";
    HEALBOT_DEBUFF_TAINTED_MIND            = "Запятнанный разум";
    HEALBOT_DEBUFF_VIPER_STING             = "Укус гадюки";
    HEALBOT_DEBUFF_SILENCE                 = "Безмолвие";
    HEALBOT_DEBUFF_MAGMA_SHACKLES          = "Оковы магмы";
    HEALBOT_DEBUFF_FROSTBOLT               = "Ледяная стрела";
    HEALBOT_DEBUFF_HUNTERS_MARK            = "Метка охотника";
    HEALBOT_DEBUFF_SLOW                    = "Замедление";
    HEALBOT_DEBUFF_ARCANE_BLAST            = "Чародейская вспышка";
    HEALBOT_DEBUFF_IMPOTENCE               = "Проклятие бессилия";
    HEALBOT_DEBUFF_DECAYED_STR             = "Ослабление силы";
    HEALBOT_DEBUFF_DECAYED_INT             = "Угасание интеллекта";
    HEALBOT_DEBUFF_CRIPPLE                 = "Увечье";
    HEALBOT_DEBUFF_CHILLED                 = "Окоченение";
    HEALBOT_DEBUFF_CONEOFCOLD              = "Конус холода";
    HEALBOT_DEBUFF_CONCUSSIVESHOT          = "Шокирующий выстрел";
    HEALBOT_DEBUFF_THUNDERCLAP             = "Раскат грома";
    HEALBOT_DEBUFF_HOWLINGSCREECH          = "Визгливый вой";
    HEALBOT_DEBUFF_DAZED                   = "Головокружение";
    HEALBOT_DEBUFF_UNSTABLE_AFFL           = "Нестабильное колдовство";
    HEALBOT_DEBUFF_DREAMLESS_SLEEP         = "Мирный сон";
    HEALBOT_DEBUFF_GREATER_DREAMLESS       = "Больший мирный сон";
    HEALBOT_DEBUFF_MAJOR_DREAMLESS         = "Старший мирный сон";
    HEALBOT_DEBUFF_FROST_SHOCK             = "Ледяной шок";
    HEALBOT_DEBUFF_WEAKENED_SOUL           = GetSpellInfo(6788);

    HEALBOT_DEBUFF_ICE_TOMB                = GetSpellInfo(29670);
    HEALBOT_DEBUFF_SACRIFICE               = GetSpellInfo(30115);
    HEALBOT_DEBUFF_ICEBOLT                 = GetSpellInfo(31249);
    HEALBOT_DEBUFF_DOOMFIRE                = GetSpellInfo(31944);
    HEALBOT_DEBUFF_IMPALING_SPINE          = GetSpellInfo(39837);
    HEALBOT_DEBUFF_FEL_RAGE                = GetSpellInfo(40604);
    HEALBOT_DEBUFF_FEL_RAGE2               = GetSpellInfo(40616);
    HEALBOT_DEBUFF_FATAL_ATTRACTION        = GetSpellInfo(41001);
    HEALBOT_DEBUFF_AGONIZING_FLAMES        = GetSpellInfo(40932);
    HEALBOT_DEBUFF_DARK_BARRAGE            = GetSpellInfo(40585);
    HEALBOT_DEBUFF_PARASITIC_SHADOWFIEND   = GetSpellInfo(41917);
    HEALBOT_DEBUFF_GRIEVOUS_THROW          = GetSpellInfo(43093);
    HEALBOT_DEBUFF_BURN                    = GetSpellInfo(46394);
    HEALBOT_DEBUFF_ENCAPSULATE             = GetSpellInfo(45662);
    HEALBOT_DEBUFF_CONFLAGRATION           = GetSpellInfo(45342);
    HEALBOT_DEBUFF_FLAME_SEAR              = GetSpellInfo(46771);
    HEALBOT_DEBUFF_FIRE_BLOOM              = GetSpellInfo(45641);
    HEALBOT_DEBUFF_GRIEVOUS_BITE           = GetSpellInfo(48920);
    HEALBOT_DEBUFF_FROST_TOMB              = GetSpellInfo(25168);
    HEALBOT_DEBUFF_IMPALE                  = GetSpellInfo(67478);
    HEALBOT_DEBUFF_WEB_WRAP                = GetSpellInfo(28622);
    HEALBOT_DEBUFF_JAGGED_KNIFE            = GetSpellInfo(55550);
    HEALBOT_DEBUFF_FROST_BLAST             = GetSpellInfo(27808);
    HEALBOT_DEBUFF_SLAG_PIT                = GetSpellInfo(63477);
    HEALBOT_DEBUFF_GRAVITY_BOMB            = GetSpellInfo(64234);
    HEALBOT_DEBUFF_LIGHT_BOMB              = GetSpellInfo(63018);
    HEALBOT_DEBUFF_STONE_GRIP              = GetSpellInfo(64292);
    HEALBOT_DEBUFF_FERAL_POUNCE            = GetSpellInfo(64669);
    HEALBOT_DEBUFF_NAPALM_SHELL            = GetSpellInfo(63666);
    HEALBOT_DEBUFF_IRON_ROOTS              = GetSpellInfo(62283);
    HEALBOT_DEBUFF_SARA_BLESSING           = GetSpellInfo(63134);
    HEALBOT_DEBUFF_SNOBOLLED               = GetSpellInfo(66406);
    HEALBOT_DEBUFF_FIRE_BOMB               = GetSpellInfo(67475);
    HEALBOT_DEBUFF_BURNING_BILE            = GetSpellInfo(66869);
    HEALBOT_DEBUFF_PARALYTIC_TOXIN         = GetSpellInfo(67618);
    HEALBOT_DEBUFF_INCINERATE_FLESH        = GetSpellInfo(67049);
    HEALBOT_DEBUFF_LEGION_FLAME            = GetSpellInfo(68123);
    HEALBOT_DEBUFF_MISTRESS_KISS           = GetSpellInfo(67078);
    HEALBOT_DEBUFF_SPINNING_PAIN_SPIKE     = GetSpellInfo(66283);
    HEALBOT_DEBUFF_TOUCH_OF_LIGHT          = GetSpellInfo(67297);
    HEALBOT_DEBUFF_TOUCH_OF_DARKNESS       = GetSpellInfo(66001);
    HEALBOT_DEBUFF_PENETRATING_COLD        = GetSpellInfo(66013);
    HEALBOT_DEBUFF_ACID_DRENCHED_MANDIBLES = GetSpellInfo(67861);
    HEALBOT_DEBUFF_EXPOSE_WEAKNESS         = GetSpellInfo(67847);
    HEALBOT_DEBUFF_IMPALED                 = GetSpellInfo(69065);
    HEALBOT_DEBUFF_NECROTIC_STRIKE         = GetSpellInfo(70659);
    HEALBOT_DEBUFF_FALLEN_CHAMPION         = GetSpellInfo(72293);
    HEALBOT_DEBUFF_BOILING_BLOOD           = GetSpellInfo(72385);
    HEALBOT_DEBUFF_RUNE_OF_BLOOD           = GetSpellInfo(72409);
    HEALBOT_DEBUFF_VILE_GAS                = GetSpellInfo(72273);
    HEALBOT_DEBUFF_GASTRIC_BLOAT           = GetSpellInfo(72219);
    HEALBOT_DEBUFF_GAS_SPORE               = GetSpellInfo(69278);
    HEALBOT_DEBUFF_INOCULATED              = GetSpellInfo(72103);
    HEALBOT_DEBUFF_MUTATED_INFECTION       = GetSpellInfo(71224);
    HEALBOT_DEBUFF_GASEOUS_BLOAT           = GetSpellInfo(72455);
    HEALBOT_DEBUFF_VOLATILE_OOZE           = GetSpellInfo(70447);
    HEALBOT_DEBUFF_MUTATED_PLAGUE          = GetSpellInfo(72745);
    HEALBOT_DEBUFF_GLITTERING_SPARKS       = GetSpellInfo(72796);
    HEALBOT_DEBUFF_SHADOW_PRISON           = GetSpellInfo(72999);
    HEALBOT_DEBUFF_SWARMING_SHADOWS        = GetSpellInfo(72638);
    HEALBOT_DEBUFF_PACT_DARKFALLEN         = GetSpellInfo(71340);
    HEALBOT_DEBUFF_ESSENCE_BLOOD_QUEEN     = GetSpellInfo(70867);
    HEALBOT_DEBUFF_DELIRIOUS_SLASH         = GetSpellInfo(71624);
    HEALBOT_DEBUFF_CORROSION               = GetSpellInfo(70751);
    HEALBOT_DEBUFF_GUT_SRPAY               = GetSpellInfo(70633);
    HEALBOT_DEBUFF_ICE_TOMB                = GetSpellInfo(70157);
    HEALBOT_DEBUFF_FROST_BEACON            = GetSpellInfo(70126);
    HEALBOT_DEBUFF_CHILLED_BONE            = GetSpellInfo(70106);
    HEALBOT_DEBUFF_INSTABILITY             = GetSpellInfo(69766);
    HEALBOT_DEBUFF_MYSTIC_BUFFET           = GetSpellInfo(70127);
    HEALBOT_DEBUFF_FROST_BREATH            = GetSpellInfo(69649);
    HEALBOT_DEBUFF_INFEST                  = GetSpellInfo(70541);
    HEALBOT_DEBUFF_NECROTIC_PLAGUE         = GetSpellInfo(70338);
    HEALBOT_DEBUFF_DEFILE                  = GetSpellInfo(72754);
    HEALBOT_DEBUFF_HARVEST_SOUL            = GetSpellInfo(68980);

    HEALBOT_RANK_1                         = "(Уровень 1)";
    HEALBOT_RANK_2                         = "(Уровень 2)";
    HEALBOT_RANK_3                         = "(Уровень 3)";
    HEALBOT_RANK_4                         = "(Уровень 4)";
    HEALBOT_RANK_5                         = "(Уровень 5)";
    HEALBOT_RANK_6                         = "(Уровень 6)";
    HEALBOT_RANK_7                         = "(Уровень 7)";
    HEALBOT_RANK_8                         = "(Уровень 8)";
    HEALBOT_RANK_9                         = "(Уровень 9)";
    HEALBOT_RANK_10                        = "(Уровень 10)";
    HEALBOT_RANK_11                        = "(Уровень 11)";
    HEALBOT_RANK_12                        = "(Уровень 12)";
    HEALBOT_RANK_13                        = "(Уровень 13)";
    HEALBOT_RANK_14                        = "(Уровень 14)";
    HEALBOT_RANK_15                        = "(Уровень 15)";
    HEALBOT_RANK_16                        = "(Уровень 16)";
    HEALBOT_RANK_17                        = "(Уровень 17)";
    HEALBOT_RANK_18                        = "(Уровень 18)";

    HB_SPELL_PATTERN_LESSER_HEAL           = "Восстанавливает (%d+) - (%d+) ед. здоровья цели.";
    HB_SPELL_PATTERN_HEAL                  = "Восстанавливает (%d+) - (%d+) ед. здоровья цели.";
    HB_SPELL_PATTERN_GREATER_HEAL          =
    "Медленно накладываемое заклинание, которое излечивает цель на (%d+)-(%d+) ед. здоровья.";
    HB_SPELL_PATTERN_FLASH_HEAL            = "Исцеляет дружественную цель на (%d+) - (%d+) ед.";
    HB_SPELL_PATTERN_RENEW1                = "Излечение (%d+) ед. урона раз в (%d+) сек";
    HB_SPELL_PATTERN_RENEW                 = "Излечивает (%d+) - (%d+) ед. здоровья цели в течение (%d+) сек";
    -- HB_SPELL_PATTERN_RENEW2              = "Not needed for en";
    -- HB_SPELL_PATTERN_RENEW3              = "Not needed for en";
    HB_SPELL_PATTERN_SHIELD                = "поглощает (%d+) урона. В течении (%d+) сек.";
    HB_SPELL_PATTERN_HEALING_TOUCH         = "Исцеляет дружественную цель на (%d+) - (%d+) ед.";
    HB_SPELL_PATTERN_REGROWTH              =
    "Исцеляет дружественную цель на (%d+) - (%d+) ед. моментально и еще на (%d+) ед. в течение (%d+) сек";
    HB_SPELL_PATTERN_REGROWTH1             = "Исцеление на (%d+) - (%d+) ед. раз в (%d+) сек.";
    HB_SPELL_PATTERN_HOLY_LIGHT            = "Исцеляет дружественную цель на (%d+) - (%d+) ед.";
    HB_SPELL_PATTERN_FLASH_OF_LIGHT        = "Исцеляет дружественную цель на (%d+) - (%d+) ед.";
    HB_SPELL_PATTERN_HEALING_WAVE          = "Исцеляет дружественную цель на (%d+) - (%d+) ед.";
    HB_SPELL_PATTERN_LESSER_HEALING_WAVE   = "Исцеляет дружественную цель на (%d+)-(%d+) ед.";
    HB_SPELL_PATTERN_REJUVENATION          = "Излечение цели на (%d+) ед. в течение (%d+) сек";
    HB_SPELL_PATTERN_REJUVENATION1         = "Излечение цели на (%d+) - (%d+) ед. в течение (%d+) сек";
    HB_SPELL_PATTERN_REJUVENATION2         = "Излечение (%d+) ед. урона раз в (%d+) с";
    HB_SPELL_PATTERN_MEND_PET              = "Восполнение здоровья питомца на (%d+) ед. в течение (%d+) сек";

    HB_TOOLTIP_MANA                        = "^Мана: (%d+)$";
    HB_TOOLTIP_RANGE                       = "Радиус действия: (%d+) м";
    HB_TOOLTIP_INSTANT_CAST                = SPELL_CAST_TIME_INSTANT_NO_MANA;
    HB_TOOLTIP_CAST_TIME                   = "Применение: (%d+.?%d*) сек";
    HB_TOOLTIP_CHANNELED                   = SPELL_CAST_CHANNELED;
    HB_TOOLTIP_OFFLINE                     = PLAYER_OFFLINE;
    HB_OFFLINE                             = PLAYER_OFFLINE;     -- has gone offline msg
    HB_ONLINE                              = GUILD_ONLINE_LABEL; -- has come online msg

    -----------------
    -- Translation --
    -----------------

    HEALBOT_ADDON                          = "HealBot " .. HEALBOT_VERSION;
    HEALBOT_LOADED                         = " загружен.";

    HEALBOT_ACTION_OPTIONS                 = "Настройки";

    HEALBOT_OPTIONS_TITLE                  = HEALBOT_ADDON;
    HEALBOT_OPTIONS_DEFAULTS               = "Стандарт";
    HEALBOT_OPTIONS_CLOSE                  = "Закрыть";
    HEALBOT_OPTIONS_HARDRESET              = "Перез.UI"
    HEALBOT_OPTIONS_SOFTRESET              = "СбросHB"
    HEALBOT_OPTIONS_INFO                   = "Инфо"
    HEALBOT_OPTIONS_TAB_GENERAL            = "Общее";
    HEALBOT_OPTIONS_TAB_SPELLS             = "Заклинания";
    HEALBOT_OPTIONS_TAB_HEALING            = "Исцеление";
    HEALBOT_OPTIONS_TAB_CDC                = "Излечение";
    HEALBOT_OPTIONS_TAB_SKIN               = "Шкурки";
    HEALBOT_OPTIONS_TAB_BUFFS              = "Баффы";

    HEALBOT_OPTIONS_PANEL_TEXT             = "Настройки панели исцеления";
    HEALBOT_OPTIONS_BARALPHA               = "Прозрач-ть включеных";
    HEALBOT_OPTIONS_BARALPHAINHEAL         = "Прозр-ть вход-го исцеления";
    HEALBOT_OPTIONS_BARALPHAEOR            = "Прозр-ть 'Вне досягаемости'";
    HEALBOT_OPTIONS_ACTIONLOCKED           = "Закрепить позиции";
    HEALBOT_OPTIONS_AUTOSHOW               = "Авто закрыть";
    HEALBOT_OPTIONS_PANELSOUNDS            = "Звук при открытии";
    HEALBOT_OPTIONS_HIDEOPTIONS            = "Скрыть кнопку настроек";
    HEALBOT_OPTIONS_PROTECTPVP             = "Избегать случайного PvP флажка";
    HEALBOT_OPTIONS_HEAL_CHATOPT           = "Настройки чата";

    HEALBOT_OPTIONS_SKINTEXT               = "Шкурка";
    HEALBOT_SKINS_STD                      = "Стандарт";
    HEALBOT_OPTIONS_SKINTEXTURE            = "Текстура";
    HEALBOT_OPTIONS_SKINHEIGHT             = "Высота";
    HEALBOT_OPTIONS_SKINWIDTH              = "Ширина";
    HEALBOT_OPTIONS_SKINNUMCOLS            = "Колонок";
    HEALBOT_OPTIONS_SKINNUMHCOLS           = "Заголовки/колонки";
    HEALBOT_OPTIONS_SKINBRSPACE            = "Промежуток строк";
    HEALBOT_OPTIONS_SKINBCSPACE            = "Промежуток рядов";
    HEALBOT_OPTIONS_EXTRASORT              = "Сорт панелей";
    HEALBOT_SORTBY_NAME                    = "Имя";
    HEALBOT_SORTBY_CLASS                   = "Класс";
    HEALBOT_SORTBY_GROUP                   = "Группа";
    HEALBOT_SORTBY_MAXHEALTH               = "Макс здоровья";
    HEALBOT_OPTIONS_NEWDEBUFFTEXT          = "Нов.Дебафф"
    HEALBOT_OPTIONS_DELSKIN                = "Удалить";
    HEALBOT_OPTIONS_NEWSKINTEXT            = "Нов.Шкурка";
    HEALBOT_OPTIONS_SAVESKIN               = "Сохранить";
    HEALBOT_OPTIONS_SKINBARS               = "Опции панели";
    HEALBOT_OPTIONS_SKINPANEL              = "Цвета панели";
    HEALBOT_SKIN_ENTEXT                    = "Включить";
    HEALBOT_SKIN_DISTEXT                   = "Выключить";
    HEALBOT_SKIN_DEBTEXT                   = "Дебафф";
    HEALBOT_SKIN_BACKTEXT                  = "Фон";
    HEALBOT_SKIN_BORDERTEXT                = "Края";
    HEALBOT_OPTIONS_SKINFONT               = "Шрифт"
    HEALBOT_OPTIONS_SKINFHEIGHT            = "Размер шрифта";
    HEALBOT_OPTIONS_BARALPHADIS            = "Прозрачностьть откл.";
    HEALBOT_OPTIONS_SHOWHEADERS            = "Заголовки";

    HEALBOT_OPTIONS_ITEMS                  = "Предметы";
    HEALBOT_OPTIONS_SPELLS                 = "Заклинания";

    HEALBOT_OPTIONS_COMBOCLASS             = "Клавиши для";
    HEALBOT_OPTIONS_CLICK                  = "Клик";
    HEALBOT_OPTIONS_SHIFT                  = "Shift";
    HEALBOT_OPTIONS_CTRL                   = "Ctrl";
    HEALBOT_OPTIONS_ENABLEHEALTHY          = "Всегда включено";

    HEALBOT_OPTIONS_CASTNOTIFY1            = "Не сообщать";
    HEALBOT_OPTIONS_CASTNOTIFY2            = "Только себе";
    HEALBOT_OPTIONS_CASTNOTIFY3            = "Извещать цель";
    HEALBOT_OPTIONS_CASTNOTIFY4            = "Группа";
    HEALBOT_OPTIONS_CASTNOTIFY5            = "Рейд";
    HEALBOT_OPTIONS_CASTNOTIFY6            = "В канал";
    HEALBOT_OPTIONS_CASTNOTIFYRESONLY      = "Извещать только воскрешение";
    HEALBOT_OPTIONS_TARGETWHISPER          = "Шепнуть цели при исцелении";

    HEALBOT_OPTIONS_CDCBARS                = "Цвета полос";
    HEALBOT_OPTIONS_CDCSHOWHBARS           = "На полосе здоровья";
    HEALBOT_OPTIONS_CDCSHOWABARS           = "На полосе угрозы";
    HEALBOT_OPTIONS_CDCCLASS               = "Монитор классов";
    HEALBOT_OPTIONS_CDCWARNINGS            = "Предупреждения о дебаффах";
    HEALBOT_OPTIONS_SHOWDEBUFFICON         = "Иконки дебаффов";
    HEALBOT_OPTIONS_SHOWDEBUFFWARNING      = "Сообщения о дебаффах";
    HEALBOT_OPTIONS_SOUNDDEBUFFWARNING     = "Звук при дебаффе";
    HEALBOT_OPTIONS_SOUND                  = "Звук"
    HEALBOT_OPTIONS_SOUND1                 = "Звук 1"
    HEALBOT_OPTIONS_SOUND2                 = "Звук 2"
    HEALBOT_OPTIONS_SOUND3                 = "Звук 3"

    HEALBOT_OPTIONS_HEAL_BUTTONS           = "Панели исцелений:";
    HEALBOT_OPTIONS_SELFHEALS              = "Себя"
    HEALBOT_OPTIONS_PETHEALS               = "Питомцев"
    HEALBOT_OPTIONS_GROUPHEALS             = "Группа";
    HEALBOT_OPTIONS_TANKHEALS              = "Главных танков";
    HEALBOT_OPTIONS_MAINASSIST             = "Наводчик";
    HEALBOT_OPTIONS_TARGETHEALS            = "Цели";
    HEALBOT_OPTIONS_EMERGENCYHEALS         = "Рейд";
    HEALBOT_OPTIONS_ALERTLEVEL             = "Тревога";
    HEALBOT_OPTIONS_EMERGFILTER            = "Экстра полосы для";
    HEALBOT_OPTIONS_EMERGFCLASS            = "Настройка классов для";
    HEALBOT_OPTIONS_COMBOBUTTON            = "Кнопка";
    HEALBOT_OPTIONS_BUTTONLEFT             = "Левая";
    HEALBOT_OPTIONS_BUTTONMIDDLE           = "Средняя";
    HEALBOT_OPTIONS_BUTTONRIGHT            = "Правая";
    HEALBOT_OPTIONS_BUTTON4                = "Кнопка4";
    HEALBOT_OPTIONS_BUTTON5                = "Кнопка5";
    HEALBOT_OPTIONS_BUTTON6                = "Кнопка6";
    HEALBOT_OPTIONS_BUTTON7                = "Кнопка7";
    HEALBOT_OPTIONS_BUTTON8                = "Кнопка8";
    HEALBOT_OPTIONS_BUTTON9                = "Кнопка9";
    HEALBOT_OPTIONS_BUTTON10               = "Кнопка10";
    HEALBOT_OPTIONS_BUTTON11               = "Кнопка11";
    HEALBOT_OPTIONS_BUTTON12               = "Кнопка12";
    HEALBOT_OPTIONS_BUTTON13               = "Кнопка13";
    HEALBOT_OPTIONS_BUTTON14               = "Кнопка14";
    HEALBOT_OPTIONS_BUTTON15               = "Кнопка15";


    HEALBOT_CLASSES_ALL                    = "Все классы";
    HEALBOT_CLASSES_MELEE                  = "Ближние";
    HEALBOT_CLASSES_RANGES                 = "Дальние";
    HEALBOT_CLASSES_HEALERS                = "Целители";
    HEALBOT_CLASSES_CUSTOM                 = "Клиентские";

    HEALBOT_OPTIONS_SHOWTOOLTIP            = "Подсказки";
    HEALBOT_OPTIONS_SHOWDETTOOLTIP         = "Инфо о заклинании";
    HEALBOT_OPTIONS_SHOWUNITTOOLTIP        = "Инфо о цели";
    HEALBOT_OPTIONS_SHOWRECTOOLTIP         = "Рекомендации Исцеления За Время";
    HEALBOT_OPTIONS_SHOWPDCTOOLTIP         = "Назначение клавиш";
    HEALBOT_TOOLTIP_POSDEFAULT             = "По умолчанию";
    HEALBOT_TOOLTIP_POSLEFT                = "Слева Healbotа";
    HEALBOT_TOOLTIP_POSRIGHT               = "Справа Healbotа";
    HEALBOT_TOOLTIP_POSABOVE               = "Вверху Healbotа";
    HEALBOT_TOOLTIP_POSBELOW               = "Внизу Healbotа";
    HEALBOT_TOOLTIP_POSCURSOR              = "Под Курсором";
    HEALBOT_TOOLTIP_RECOMMENDTEXT          = "Рекомендации Исцеления За Время";
    HEALBOT_TOOLTIP_NONE                   = "нет доступных";
    HEALBOT_TOOLTIP_ITEMBONUS              = "Сила заклинаний";
    HEALBOT_TOOLTIP_ACTUALBONUS            = "Актуальный бонус";
    HEALBOT_TOOLTIP_SHIELD                 = "Защита";
    HEALBOT_TOOLTIP_LOCATION               = "Локация";
    HEALBOT_TOOLTIP_CORPSE                 = "Труп ";
    HEALBOT_WORDS_OVER                     = "в течение";
    HEALBOT_WORDS_SEC                      = "сек";
    HEALBOT_WORDS_TO                       = "-";
    HEALBOT_WORDS_CAST                     = "Применение";
    HEALBOT_WORDS_FOR                      = "для";
    HEALBOT_WORDS_UNKNOWN                  = "неизвестно";
    HEALBOT_WORDS_YES                      = "Да";
    HEALBOT_WORDS_NO                       = "Нет";

    HEALBOT_WORDS_NONE                     = "Нету";
    HEALBOT_OPTIONS_ALT                    = "Alt";
    HEALBOT_DISABLED_TARGET                = "Цель"
    HEALBOT_OPTIONS_SHOWCLASSONBAR         = "Классы на панели";
    HEALBOT_OPTIONS_SHOWHEALTHONBAR        = "Здоровье на панели";
    HEALBOT_OPTIONS_BARHEALTHINCHEALS      = "Входящее исцеление";
    HEALBOT_OPTIONS_BARHEALTHSEPHEALS      = "Разделять вход. исцеление";
    HEALBOT_OPTIONS_BARHEALTH1             = "в цифрах";
    HEALBOT_OPTIONS_BARHEALTH2             = "в процентах";
    HEALBOT_OPTIONS_TIPTEXT                = "Информация в подсказке";
    HEALBOT_OPTIONS_BARINFOTEXT            = "Инфо панель";
    HEALBOT_OPTIONS_POSTOOLTIP             = "Подсказка";
    HEALBOT_OPTIONS_SHOWNAMEONBAR          = "Имена";
    HEALBOT_SKIN_BARCLASSCUSTOMCOLOUR      = "Окраска панели по";
    HEALBOT_OPTIONS_BARTEXTCLASSCOLOUR1    = "Текст по цвету класса";
    HEALBOT_OPTIONS_EMERGFILTERGROUPS      = "Группы";

    HEALBOT_ONE                            = "1";
    HEALBOT_TWO                            = "2";
    HEALBOT_THREE                          = "3";
    HEALBOT_FOUR                           = "4";
    HEALBOT_FIVE                           = "5";
    HEALBOT_SIX                            = "6";
    HEALBOT_SEVEN                          = "7";
    HEALBOT_EIGHT                          = "8";

    HEALBOT_OPTIONS_SETDEFAULTS            = "Стандарт";
    HEALBOT_OPTIONS_SETDEFAULTSMSG         = "Сброс всех настроек на стандартные";
    HEALBOT_OPTIONS_RIGHTBOPTIONS          = "ПКМ открывает настройки";

    HEALBOT_OPTIONS_HEADEROPTTEXT          = "Заголовки";
    HEALBOT_OPTIONS_ICONOPTTEXT            = "Опции иконки";
    HEALBOT_SKIN_HEADERBARCOL              = "Цвета панелей";
    HEALBOT_SKIN_HEADERTEXTCOL             = "Цвета текста";
    HEALBOT_OPTIONS_BUFFSTEXT1             = "Заклинание";
    HEALBOT_OPTIONS_BUFFSTEXT2             = "Проверка";
    HEALBOT_OPTIONS_BUFFSTEXT3             = "Цвета панелей";
    HEALBOT_OPTIONS_BUFF                   = "Бафф";
    HEALBOT_OPTIONS_BUFFSELF               = "на себя";
    HEALBOT_OPTIONS_BUFFPARTY              = "на группу";
    HEALBOT_OPTIONS_BUFFRAID               = "на рейд";
    HEALBOT_OPTIONS_MONITORBUFFS           = "Монитор пропущенных баффов";
    HEALBOT_OPTIONS_MONITORBUFFSC          = "также в бою";
    HEALBOT_OPTIONS_ENABLESMARTCAST        = "БыстроеЧтение когда в не боя";
    HEALBOT_OPTIONS_SMARTCASTSPELLS        = "Заклинания";
    HEALBOT_OPTIONS_SMARTCASTDISPELL       = "Расс-ть дебафф";
    HEALBOT_OPTIONS_SMARTCASTBUFF          = "Добавить бафф";
    HEALBOT_OPTIONS_SMARTCASTHEAL          = "Исцеления";
    HEALBOT_OPTIONS_BAR2SIZE               = "Размер полосы маны";
    HEALBOT_OPTIONS_SETSPELLS              = "Заклинания";
    HEALBOT_OPTIONS_ENABLEDBARS            = "Включить панели";
    HEALBOT_OPTIONS_DISABLEDBARS           = "Отключить панели когда в не боя";
    HEALBOT_OPTIONS_MONITORDEBUFFS         = "Монитор снятия дебаффов";
    HEALBOT_OPTIONS_DEBUFFTEXT1            = "Зак-ние снимающее дебаффы";

    HEALBOT_OPTIONS_IGNOREDEBUFF           = "Игнорировать:";
    HEALBOT_OPTIONS_IGNOREDEBUFFCLASS      = "По классам";
    HEALBOT_OPTIONS_IGNOREDEBUFFMOVEMENT   = "Замедления";
    HEALBOT_OPTIONS_IGNOREDEBUFFDURATION   = "Срок действия";
    HEALBOT_OPTIONS_IGNOREDEBUFFNOHARM     = "Не вредные";

    HEALBOT_OPTIONS_RANGECHECKFREQ         = "Проверка досягаемости";
    HEALBOT_OPTIONS_RANGECHECKUNITS        = "Макс. раненных целей за проверку досег-ти"

    HEALBOT_OPTIONS_HIDEPARTYFRAMES        = "Скрыть окна группы";
    HEALBOT_OPTIONS_HIDEPLAYERTARGET       = "Включая игрока и цель";
    HEALBOT_OPTIONS_DISABLEHEALBOT         = "Отключить HealBot";

    HEALBOT_OPTIONS_CHECKEDTARGET          = "Отмеченный";

    HEALBOT_ASSIST                         = "Assist";
    HEALBOT_FOCUS                          = "Focus";
    HEALBOT_MENU                           = "Меню";
    HEALBOT_MAINTANK                       = "Г.Танк";
    HEALBOT_MAINASSIST                     = "Г.Помощник";
    HEALBOT_STOP                           = "Стоп";
    HEALBOT_TELL                           = "Сказать";

    HEALBOT_TITAN_SMARTCAST                = "БыстроеЧтение";
    HEALBOT_TITAN_MONITORBUFFS             = "Монитор баффов";
    HEALBOT_TITAN_MONITORDEBUFFS           = "Монитор дебаффов"
    HEALBOT_TITAN_SHOWBARS                 = "Панели для";
    HEALBOT_TITAN_EXTRABARS                = "Доп. панели";
    HEALBOT_BUTTON_TOOLTIP                 = "ЛКМ переключает окно настроек HealBotа\nПКМ (Удерживая) для перемещения";
    HEALBOT_TITAN_TOOLTIP                  = "ЛКМ переключает окно настроек HealBotа";
    HEALBOT_OPTIONS_SHOWMINIMAPBUTTON      = "Показ кнопки у мини-карты";
    HEALBOT_OPTIONS_BARBUTTONSHOWHOT       = "Показ иконки ИзВ";
    HEALBOT_OPTIONS_BARBUTTONSHOWRAIDICON  = "Показ иконки рейда";
    HEALBOT_OPTIONS_HOTONBAR               = "Внутри";
    HEALBOT_OPTIONS_HOTOFFBAR              = "Снаружи";
    HEALBOT_OPTIONS_HOTBARRIGHT            = "Справа";
    HEALBOT_OPTIONS_HOTBARLEFT             = "Слева";

    HEALBOT_OPTIONS_ENABLETARGETSTATUS     = "Включенные настройки когда цель в бою";

    HEALBOT_ZONE_AB                        = "Низина Арати";
    HEALBOT_ZONE_AV                        = "Альтеракская долина";
    HEALBOT_ZONE_ES                        = "Око Бури";
    HEALBOT_ZONE_IC                        = "Остров Завоеваний";
    HEALBOT_ZONE_SA                        = "Берег Древних";
    HEALBOT_ZONE_WG                        = "Ущелье Песни Войны";

    HEALBOT_OPTION_AGGROTRACK              = "Монитор агрессии";
    HEALBOT_OPTION_AGGROBAR                = "Мигание";
    HEALBOT_OPTION_AGGROTXT                = ">> Показ текста <<";
    HEALBOT_OPTION_BARUPDFREQ              = "Частота обновления";
    HEALBOT_OPTION_USEFLUIDBARS            = "Исп текучие полосы";
    HEALBOT_OPTION_CPUPROFILE              = "Использовать профайлер CPU (Инфо о нагрузке CPU )";
    HEALBOT_OPTIONS_RELOADUIMSG            =
    "Для того чтобы настройки вступили бы в силу необходима перезагрузка интерфейса, Готовы?";

    HEALBOT_SELF_PVP                       = "Своё ПвП";
    HEALBOT_OPTIONS_ANCHOR                 = "Якорь фрейма";
    HEALBOT_OPTIONS_BARSANCHOR             = "Якорь полос"
    HEALBOT_OPTIONS_TOPLEFT                = "Вверху слева";
    HEALBOT_OPTIONS_BOTTOMLEFT             = "Внизу слева";
    HEALBOT_OPTIONS_TOPRIGHT               = "Вверху справа";
    HEALBOT_OPTIONS_BOTTOMRIGHT            = "Внизу справа";
    HEALBOT_OPTIONS_TOP                    = "Вверху";
    HEALBOT_OPTIONS_BOTTOM                 = "Внизу";

    HEALBOT_PANEL_BLACKLIST                = "Чёрный-Список";

    HEALBOT_WORDS_REMOVEFROM               = "Снять с";
    HEALBOT_WORDS_ADDTO                    = "Наложить на";
    HEALBOT_WORDS_INCLUDE                  = "Включая";

    HEALBOT_OPTIONS_TTALPHA                = "Прозрачность";
    HEALBOT_TOOLTIP_TARGETBAR              = "Панель цели";
    HEALBOT_OPTIONS_MYTARGET               = "Моя цель";

    HEALBOT_DISCONNECTED_TEXT              = "<ОФФ>"
    HEALBOT_OPTIONS_SHOWUNITBUFFTIME       = "Показ.мои баффы";
    HEALBOT_OPTIONS_TOOLTIPUPDATE          = "Постоянно обновлять";
    HEALBOT_OPTIONS_BUFFSTEXTTIMER         = "Показ бафф перед окончанием";
    HEALBOT_OPTIONS_SHORTBUFFTIMER         = "Короткие баффы"
    HEALBOT_OPTIONS_LONGBUFFTIMER          = "Длинные баффы"

    HEALBOT_BALANCE                        = "Баланс"
    HEALBOT_FERAL                          = "Сила зверя"
    HEALBOT_RESTORATION                    = "Исцеление"
    HEALBOT_SHAMAN_RESTORATION             = "Исцеление"
    HEALBOT_ARCANE                         = "Тайная магия"
    HEALBOT_FIRE                           = "Огонь"
    HEALBOT_FROST                          = "Лед"
    HEALBOT_DISCIPLINE                     = "Послушание"
    HEALBOT_HOLY                           = "Свет"
    HEALBOT_SHADOW                         = "Темная магия"
    HEALBOT_ASSASSINATION                  = "Убийство"
    HEALBOT_COMBAT                         = "Бой"
    HEALBOT_SUBTLETY                       = "Скрытность"
    HEALBOT_ARMS                           = "Оружие"
    HEALBOT_FURY                           = "Неистовство"
    HEALBOT_PROTECTION                     = "Защита"
    HEALBOT_BEASTMASTERY                   = "Чувство зверя"
    HEALBOT_MARKSMANSHIP                   = "Стрельба"
    HEALBOT_SURVIVAL                       = "Выживание"
    HEALBOT_RETRIBUTION                    = "Возмездие"
    HEALBOT_ELEMENTAL                      = "Укрощение стихии"
    HEALBOT_ENHANCEMENT                    = "Совершенствование"
    HEALBOT_AFFLICTION                     = "Колдовство"
    HEALBOT_DEMONOLOGY                     = "Демонология"
    HEALBOT_DESTRUCTION                    = "Разрушение"
    HEALBOT_BLOOD                          = "Кровь"
    HEALBOT_UNHOLY                         = "Нечестивость"

    HEALBOT_OPTIONS_VISIBLERANGE           = "Отключать панель когда расстояние больше 100 метров"
    HEALBOT_OPTIONS_NOTIFY_HEAL_MSG        = "Сообщения о исцелении"
    HEALBOT_OPTIONS_NOTIFY_OTHER_MSG       = "Другие сообщения"
    HEALBOT_WORDS_YOU                      = "вы";
    HEALBOT_NOTIFYHEALMSG                  = "Применяется #s для исцеления #n на #h";
    HEALBOT_NOTIFYOTHERMSG                 = "Применяется #s на #n";

    HEALBOT_OPTIONS_HOTPOSITION            = "Позиция иконки"
    HEALBOT_OPTIONS_HOTSHOWTEXT            = "Текст иконки"
    HEALBOT_OPTIONS_HOTTEXTCOUNT           = "Подсчёт"
    HEALBOT_OPTIONS_HOTTEXTDURATION        = "Длительность"
    HEALBOT_OPTIONS_ICONSCALE              = "Масштаб"
    HEALBOT_OPTIONS_ICONTEXTSCALE          = "Масштаб текста иконки"

    HEALBOT_SKIN_FLUID                     = "Fluid"
    HEALBOT_SKIN_VIVID                     = "Vivid"
    HEALBOT_SKIN_LIGHT                     = "Light"
    HEALBOT_SKIN_SQUARE                    = "Квадраты"
    HEALBOT_OPTIONS_AGGROBARSIZE           = "Размер полосы угрозы"
    HEALBOT_OPTIONS_TARGETBARMODE          = "Ограничить полосу цели предписанными настройками"
    HEALBOT_OPTIONS_DOUBLETEXTLINES        = "Текст в 2 строки"
    HEALBOT_OPTIONS_TEXTALIGNMENT          = "Выравнивание текста"
    HEALBOT_OPTIONS_ENABLELIBQH            = "Включить libQuickHealth"
    HEALBOT_VEHICLE                        = "Транспорт"
    HEALBOT_OPTIONS_UNIQUESPEC             = "Спец. заклинание для другой спецификации"
    HEALBOT_WORDS_ERROR                    = "Ошибка"
    HEALBOT_SPELL_NOT_FOUND                = "Заклинание не найдено"
    HEALBOT_OPTIONS_DISABLETOOLTIPINCOMBAT = "Скрыть подсказки в бою"

    HEALBOT_OPTIONS_BUFFNAMED              = "Для наблюдения, введите имя игрока:\n\n"
    HEALBOT_OPTIONS_INHEALS                = "Исп HealBot для связи исцелений"
    HEALBOT_WORD_ALWAYS                    = "Всегда";
    HEALBOT_WORD_SOLO                      = "В одиночку";
    HEALBOT_WORD_NEVER                     = "Никогда";
    HEALBOT_SHOW_CLASS_AS_ICON             = "как иконку";
    HEALBOT_SHOW_CLASS_AS_TEXT             = "как текст";

    HEALBOT_SHOW_INCHEALS                  = "Входящее исцеление";
    HEALBOT_D_DURATION                     = "Длительность прямых";
    HEALBOT_H_DURATION                     = "Длительность ХоТов";
    HEALBOT_C_DURATION                     = "Длительность потоковых";

    HEALBOT_HELP                           = {
        [1] = "[HealBot] /hb h -- показать справку",
        [2] = "[HealBot] /hb o -- переключение настроек",
        [3] = "[HealBot] /hb d -- сброс настроек на стандартные значения",
        [4] = "[HealBot] /hb ui -- перезагрузить UI",
        [5] = "[HealBot] /hb ri -- сбросить HealBot",
        [6] = "[HealBot] /hb t -- переключения Healbot из состояния включен в выключен и обратно",
        [7] = "[HealBot] /hb skin <skinName> -- сменить шкурку",
        [8] = "[HealBot] /hb tr <Role> -- Set highest role priority for SubSort by Role. Valid Roles are 'TANK', 'HEALER' or 'DPS'",
    }

    HEALBOT_OPTION_HIGHLIGHTACTIVEBAR      = "Выделять активную полосу"
    HEALBOT_OPTIONS_TESTBARS               = "Тест полос"
    HEALBOT_OPTION_NUMBARS                 = "Количество полос"
    HEALBOT_OPTION_NUMTANKS                = "Количество танков"
    HEALBOT_OPTION_NUMMYTARGETS            = "Количество моих целей"
    HEALBOT_OPTION_NUMPETS                 = "Количество питомцев"
    HEALBOT_WORD_TEST                      = "Тест";
    HEALBOT_WORD_OFF                       = "Выкл";
    HEALBOT_WORD_ON                        = "Вкл";

    HEALBOT_OPTIONS_TAB_INCHEALS           = "Вхд. исцел."
    HEALBOT_OPTIONS_TAB_CHAT               = "Чат"
    HEALBOT_OPTIONS_TAB_HEADERS            = "Заголовки"
    HEALBOT_OPTIONS_TAB_BARS               = "Полосы"
    HEALBOT_OPTIONS_TAB_TEXT               = "Текст"
    HEALBOT_OPTIONS_TAB_ICONS              = "Иконки"
    HEALBOT_OPTIONS_TAB_WARNING            = "Оповещения"
    HEALBOT_OPTIONS_SKINDEFAULTFOR         = "Стандарт. шкурка для"
    HEALBOT_OPTIONS_INCHEAL                = "Вхд. исцеление"
    HEALBOT_WORD_ARENA                     = "Арена"
    HEALBOT_WORD_BATTLEGROUND              = "Поля боя"
    HEALBOT_OPTIONS_TEXTOPTIONS            = "Опции текста"
    HEALBOT_WORD_PARTY                     = "Группа"
    HEALBOT_OPTIONS_COMBOAUTOTARGET        = "Авто цель"
    HEALBOT_OPTIONS_COMBOAUTOTRINKET       = "Авто аксессуар"
    HEALBOT_OPTIONS_GROUPSPERCOLUMN        = "Исп. группы на колонку"

    HEALBOT_OPTIONS_MAINSORT               = "Гл. сорт"
    HEALBOT_OPTIONS_SUBSORT                = "Доп. сорт"
    HEALBOT_OPTIONS_SUBSORTINC             = "Также доп сорт:"

    HEALBOT_OPTIONS_HEALCOMMMETHOD         = "Метод связи исцеления:"
    HEALBOT_OPTIONS_HEALCOMMUSELHC         = "Включить libHealComm"
    HEALBOT_OPTIONS_HEALCOMMLHCNOTFOUND    = "libHealComm не установлена"
    HEALBOT_OPTIONS_HEALCOMMINTERNAL1      = "Внутренний без проверки (Без связи)"
    HEALBOT_OPTIONS_HEALCOMMINTERNAL2      = "Внутренний проверяя применения (Без связи)"
    HEALBOT_OPTIONS_HEALCOMMNOLHC          = "libHealComm is not embedded\n\nSome Incoming Heal sliders on this tab are not used"
    HEALBOT_OPTIONS_HEALCOMMNOLHC2         =
    "libHealComm is not enabled but still embedded\nThe library is still sending Addon comms spam\n\nTo disable see disable_libHealComm.html in the Docs directory"

    HEALBOT_OPTIONS_BUTTONCASTMETHOD       = "Применить\nкогда"
    HEALBOT_OPTIONS_BUTTONCASTPRESSED      = "Нажата"
    HEALBOT_OPTIONS_BUTTONCASTRELEASED     = "Отпущена"

    HEALBOT_INFO_INCHEALINFO               = "== Инфо входящего исцеления =="
    HEALBOT_INFO_ADDONCPUUSAGE             = "== Исп. CPU аддоном в секундах =="
    HEALBOT_INFO_ADDONCOMMUSAGE            = "== Исп. коммуникации аддоном =="
    HEALBOT_WORD_HEALER                    = "Лекарь"
    HEALBOT_WORD_VERSION                   = "Версия"
    HEALBOT_WORD_CLIENT                    = "Клиент"
    HEALBOT_WORD_ADDON                     = "Аддон"
    HEALBOT_INFO_CPUSECS                   = "CPU сек"
    HEALBOT_INFO_MEMORYKB                  = "Память КБ"
    HEALBOT_INFO_COMMS                     = "Связь КБ"

    HEALBOT_OPTIONS_HEALCOMMINTERNAL3      = "Ультра низкая связь - без проверки"
    HEALBOT_OPTIONS_HEALCOMMINTERNAL4      = "Ультра низкая связь - проверка применения"

    HEALBOT_WORD_STAR                      = "Звезда"
    HEALBOT_WORD_CIRCLE                    = "Круг"
    HEALBOT_WORD_DIAMOND                   = "Ромб"
    HEALBOT_WORD_TRIANGLE                  = "Треугольник"
    HEALBOT_WORD_MOON                      = "Луна"
    HEALBOT_WORD_SQUARE                    = "Квадрат"
    HEALBOT_WORD_CROSS                     = "Крест"
    HEALBOT_WORD_SKULL                     = "Череп"

    HEALBOT_OPTIONS_ACCEPTSKINMSG          = "Принять шкурку [HealBot]: "
    HEALBOT_OPTIONS_ACCEPTSKINMSGFROM      = " от "
    HEALBOT_OPTIONS_BUTTONSHARESKIN        = "Поделиться с"

    HEALBOT_CHAT_ADDONID                   = "[HealBot]  "
    HEALBOT_CHAT_NEWVERSION1               = "Доступна новая версия"
    HEALBOT_CHAT_NEWVERSION2               = "на https://discord.gg/addonsforsirus"
    HEALBOT_CHAT_SHARESKINERR1             = " Skin not found for Sharing"
    HEALBOT_CHAT_SHARESKINERR2             = "'s HealBot version is too old for Skin Sharing"
    HEALBOT_CHAT_SHARESKINERR3             = " not found for Skin Sharing"
    HEALBOT_CHAT_SHARESKINACPT             = "Share Skin accepted from "
    HEALBOT_CHAT_CONFIRMSKINDEFAULTS       = "Шкурка по умолчанию"
    HEALBOT_CHAT_CONFIRMCUSTOMDEFAULTS     = "Custom Debuffs reset"
    HEALBOT_CHAT_CHANGESKINERR1            = "неизвестная шкурка: /hb skin "
    HEALBOT_CHAT_CHANGESKINERR2            = "Действительные шкурки:  "
    HEALBOT_CHAT_CONFIRMSPELLCOPY          = "Current spells copied for all specs"
    HEALBOT_CHAT_UNKNOWNCMD                = "Unknown slash command: /hb "
    HEALBOT_CHAT_ENABLED                   = "Entering enabled state"
    HEALBOT_CHAT_DISABLED                  = "Entering disabled state"
    HEALBOT_CHAT_SOFTRELOAD                = "Reload healbot requested"
    HEALBOT_CHAT_HARDRELOAD                = "Reload UI requested"
    HEALBOT_CHAT_CONFIRMSPELLRESET         = "Заклинания сброшены"
    HEALBOT_CHAT_CONFIRMCURESRESET         = "Лечения сброшены"
    HEALBOT_CHAT_CONFIRMBUFFSRESET         = "Баффы сброшены"
    HEALBOT_CHAT_POSSIBLEMISSINGMEDIA      =
    "Unable to receive all Skin settings - Possibly missing SharedMedia, see HealBot/Docs/readme.html for links"
    HEALBOT_CHAT_MACROSOUNDON              = "Sound not suppressed when using auto trinkets"
    HEALBOT_CHAT_MACROSOUNDOFF             = "Sound suppressed when using auto trinkets"
    HEALBOT_CHAT_MACROERRORON              = "Errors not suppressed when using auto trinkets"
    HEALBOT_CHAT_MACROERROROFF             = "Errors suppressed when using auto trinkets"
    HEALBOT_CHAT_TITANON                   = "Titan panel - updates on"
    HEALBOT_CHAT_TITANOFF                  = "Titan panel - updates off"
    HEALBOT_CHAT_ACCEPTSKINON              = "Share Skin - Show accept skin popup when someone shares a skin with you"
    HEALBOT_CHAT_ACCEPTSKINOFF             = "Share Skin - Always ignore share skins from everyone"
    HEALBOT_CHAT_USE10ON                   = "Auto Trinket - Use10 is on - You must enable an existing auto trinket for use10 to work"
    HEALBOT_CHAT_USE10OFF                  = "Auto Trinket - Use10 is off"
    HEALBOT_CHAT_SKINREC                   = " получена шкурка от "

    HEALBOT_OPTIONS_SELFCASTS              = "Только личные применения"
    HEALBOT_OPTIONS_HOTSHOWICON            = "Иконка"
    HEALBOT_OPTIONS_ALLSPELLS              = "Все заклинания"
    HEALBOT_OPTIONS_DOUBLEROW              = "Двойной ряд"
    HEALBOT_OPTIONS_HOTBELOWBAR            = "Ниже полосы"
    HEALBOT_OPTIONS_OTHERSPELLS            = "Другие заклинания"
    HEALBOT_WORD_MACROS                    = "Макрос"
    HEALBOT_WORD_SELECT                    = "Выбрать"
    HEALBOT_OPTIONS_QUESTION               = "?"
    HEALBOT_WORD_CANCEL                    = "Отмена"
    HEALBOT_WORD_COMMANDS                  = "Commands"
    HEALBOT_OPTIONS_BARHEALTH3             = "как здоровье";
    HEALBOT_SORTBY_ROLE                    = "Роль"
    HEALBOT_WORD_DPS                       = "DPS"
    HEALBOT_CHAT_TOPROLEERR                = " роль не действительна в данном контексте - используйте 'TANK', 'DPS' или 'HEALER'"
    HEALBOT_CHAT_NEWTOPROLE                = "Highest top role is now "
    HEALBOT_CHAT_SUBSORTPLAYER1            = "Player will be set to first in SubSort"
    HEALBOT_CHAT_SUBSORTPLAYER2            = "Player will be sorted normally in SubSort"
    HEALBOT_OPTIONS_SHOWREADYCHECK         = "Проверка готовности";
    HEALBOT_OPTIONS_SUBSORTSELFFIRST       = "Сначало вы"
    HEALBOT_WORD_FILTER                    = "Фильтр"
    HEALBOT_OPTION_AGGROPCTBAR             = "Двигать"
    HEALBOT_OPTION_AGGROPCTTXT             = "Показ. текст"
    HEALBOT_OPTION_AGGROPCTTRACK           = "Следить в %"
    HEALBOT_OPTIONS_ALERTAGGROLEVEL0       = "0 - имея низкую угрозы и ничего не танкуя"
    HEALBOT_OPTIONS_ALERTAGGROLEVEL1       = "1 - имея высокую угрозу и ничего не танкуя"
    HEALBOT_OPTIONS_ALERTAGGROLEVEL2       = "2 - опасное танкования, не наивысшая угроза на существе"
    HEALBOT_OPTIONS_ALERTAGGROLEVEL3       = "3 - безопасное танкованин по крайней мере одно существо"
    HEALBOT_OPTIONS_AGGROALERT             = "Уровент сигнала об угрозе"
    HEALBOT_OPTIONS_SETAGGROERROR1         = " Invalid parameters pass to Set Aggro ..."
    HEALBOT_OPTIONS_SETAGGRORESET          = " Aggro bar settings reset"
    HEALBOT_OPTIONS_SETAGGROERROR2         = " Set Aggro min opacity failed - Valid value range is 0 to 0.8"
    HEALBOT_OPTIONS_SETAGGROERROR3         = " Set Aggro min opacity failed - Min value is greater than Max"
    HEALBOT_OPTIONS_SETAGGROERROR4         = " Set Aggro max opacity failed - Valid value range is 0.2 to 1"
    HEALBOT_OPTIONS_SETAGGROERROR5         = " Set Aggro max opacity failed - Max value is less than Min"
    HEALBOT_OPTIONS_SETAGGROMIN            = " Aggro min opacity set to "
    HEALBOT_OPTIONS_SETAGGROMAX            = " Aggro max opacity set to "
    HEALBOT_OPTIONS_SETAGGROERROR6         = " Set Aggro flash frequency failed - Valid value range is 0.005 to 0.2"
    HEALBOT_OPTIONS_SETAGGROFLASH          = " Aggro flash frequency set to "
    HEALBOT_OPTIONS_SETAGGROERROR7         = " Set Aggro opacity failed - Invalid <What End>.  Use 'Min' or 'Max'"
    HEALBOT_OPTIONS_SETAGGROERROR8         = " Invalid aggro status - use 'Low', 'High' or 'Has'"
    HEALBOT_OPTIONS_SETAGGROERROR9         = " Set Aggro Cols failed - Blue not between 0 and 1"
    HEALBOT_OPTIONS_SETAGGROERROR10        = " Set Aggro Cols failed - Green not between 0 and 1"
    HEALBOT_OPTIONS_SETAGGROERROR11        = " Set Aggro Cols failed - Red not between 0 and 1"
    HEALBOT_OPTIONS_SETAGGROCOL            = " Aggro Cols set to "
    HEALBOT_OPTIONS_TOOLTIPSHOWHOT         = "Show active monitored HoT details"
    HEALBOT_OPTIONS_SETAGGROFLASHCUR       = " Aggro flash frequency="
    HEALBOT_OPTIONS_SETAGGROALPHACUR       = " Aggro opacity "
    HEALBOT_OPTIONS_SETAGGROCOLLOW         = " Aggro cols low threat (1) "
    HEALBOT_OPTIONS_SETAGGROCOLHIGH        = " Aggro cols high threat (2) "
    HEALBOT_OPTIONS_SETAGGROCOLHAS         = " Aggro cols has aggro (3) "
    HEALBOT_WORDS_MIN                      = "мин"
    HEALBOT_WORDS_MAX                      = "макс"
    HEALBOT_WORDS_R                        = "R"
    HEALBOT_WORDS_G                        = "G"
    HEALBOT_WORDS_B                        = "B"
    HEALBOT_CHAT_SELFPETSON                = "Self Pet switched on"
    HEALBOT_CHAT_SELFPETSOFF               = "Self Pet switched off"
    HEALBOT_WORD_PRIORITY                  = "Приоритет"
    HEALBOT_VISIBLE_RANGE                  = "В пределах 100 метров"
    HEALBOT_SPELL_RANGE                    = "В пределах действия заклинания"

    HEALBOT_CUSTOM_CATEGORY                = "Категория"
    HEALBOT_CUSTOM_CAT_CUSTOM              = "Свой"
    HEALBOT_CUSTOM_CAT_CLASSIC             = "Classic"
    HEALBOT_CUSTOM_CAT_TBC_OTHER           = "TBC - Другое"
    HEALBOT_CUSTOM_CAT_TBC_BT              = "TBC - Черный храм"
    HEALBOT_CUSTOM_CAT_TBC_SUNWELL         = "TBC - Солнечный Колодец"
    HEALBOT_CUSTOM_CAT_LK_OTHER            = "WotLK - Другое"
    HEALBOT_CUSTOM_CAT_LK_ULDUAR           = "WotLK - Ульдуар"
    HEALBOT_CUSTOM_CAT_LK_TOC              = "WotLK - Испытание крестоносца"
    HEALBOT_CUSTOM_CAT_LK_ICC_LOWER        = "WotLK - ЦЛК Нижний ярус"
    HEALBOT_CUSTOM_CAT_LK_ICC_PLAGUEWORKS  = "WotLK - ЦЛК Чумодельня"
    HEALBOT_CUSTOM_CAT_LK_ICC_CRIMSON      = "WotLK - ЦЛК Багровый зал"
    HEALBOT_CUSTOM_CAT_LK_ICC_FROSTWING    = "WotLK - ЦЛК Залы Ледокрылых"
    HEALBOT_CUSTOM_CAT_LK_ICC_THRONE       = "WotLK - ЦЛК Ледяной Трон"
    HEALBOT_WORD_RESET                     = "Сброс"
    HEALBOT_HBMENU                         = "HBmenu"
    HEALBOT_ACTION_HBFOCUS                 = "Левый клик - установить\nфокус на цель"
    HEALBOT_WORD_CLEAR                     = "Очистить"
    HEALBOT_WORD_SET                       = "Установить"
    HEALBOT_WORD_HBFOCUS                   = "Фокус HealBot"
    HEALBOT_WORD_OUTSIDE                   = "Внешний мир"
    HEALBOT_WORD_ALLZONE                   = "Все зоны"



    HEALBOT_OPTIONS_TAB_ALERT        = "Тревога"
    HEALBOT_OPTIONS_TAB_SORT         = "Сорт"
    HEALBOT_OPTIONS_TAB_AGGRO        = "Угроза"
    HEALBOT_OPTIONS_TAB_ICONTEXT     = "Текст иконки"
    HEALBOT_OPTIONS_TAB_TEXT         = "Текст панели"
    HEALBOT_OPTIONS_AGGROBARCOLS     = "Цвета полосы угрозы";
    HEALBOT_OPTIONS_AGGRO1COL        = "Высокая\nугроза"
    HEALBOT_OPTIONS_AGGRO2COL        = "Опасное\nтанкование"
    HEALBOT_OPTIONS_AGGRO3COL        = "Безопастное\nтанкование"
    HEALBOT_OPTIONS_AGGROFLASHFREQ   = "Частота сверкания"
    HEALBOT_OPTIONS_AGGROFLASHALPHA  = "Прозрачность сверкания"
    HEALBOT_OPTIONS_SHOWDURATIONFROM = "Длительность от"
    HEALBOT_OPTIONS_SHOWDURATIONWARN = "Оповещение длительности от"
    HEALBOT_CMD_RESETCUSTOMDEBUFFS   = "Сброс своих дебаффов"
    HEALBOT_CMD_RESETSKINS           = "Сброс шкурок"
    HEALBOT_CMD_CLEARBLACKLIST       = "Очистить черный список"
    HEALBOT_CMD_TOGGLEACCEPTSKINS    = "Переключение приёма шкурок от других"
    HEALBOT_CMD_COPYSPELLS           = "Копировать данное заклинание во все наборы"
    HEALBOT_CMD_RESETSPELLS          = "Сброс заклинаний"
    HEALBOT_CMD_RESETCURES           = "Сброс лечений"
    HEALBOT_CMD_RESETBUFFS           = "Сброс баффов"
    HEALBOT_CMD_RESETBARS            = "Сброс позиций панелей"
    HEALBOT_CMD_TOGGLETITAN          = "Переключить обновления титана"
    HEALBOT_CMD_SUPPRESSSOUND        = "Переключение сдерживания звука когда используется авто аксессуар"
    HEALBOT_CMD_SUPPRESSERRORS       = "Переключение сдерживания ошибок когда используется авто аксессуар"
    HEALBOT_OPTIONS_COMMANDS         = "Команды HealBot"
    HEALBOT_WORD_RUN                 = "Пуск"
    HEALBOT_OPTIONS_MOUSEWHEEL       = "Включит меню на колесике мыши"
    HEALBOT_CMD_DELCUSTOMDEBUFF10    = "Delete custom debuffs on priority 10"
    HEALBOT_ACCEPTSKINS              = "Принимать шкурки от других"
    HEALBOT_SUPPRESSSOUND            = "Авто-Аксессуар: Подавлять звуки"
    HEALBOT_SUPPRESSERROR            = "Авто-Аксессуар: Подавлять ошибки"
end
